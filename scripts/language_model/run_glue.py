...
parser.add_argument('--lr_decay', type=str, choices=['linear'], default='linear',
                    help='lr schedule')
...
def preprocess_data(_tokenizer, _task, batch_size, dev_batch_size, max_len, _vocab):
    """Train/eval Data preparation function."""
    label_dtype = 'int32' if _task.class_labels else 'float32'
    truncate_length = max_len - 3 if _task.is_pair else max_len - 2
    trans = partial(convert_examples_to_features, tokenizer=_tokenizer,
                    truncate_length=truncate_length, cls_token=_vocab.cls_token,
                    sep_token=_vocab.sep_token, class_labels=_task.class_labels,
                    label_alias=_task.label_alias, vocab=_vocab)
    ...
    test_trans = partial(convert_examples_to_features, tokenizer=_tokenizer,
                         truncate_length=max_len, cls_token=_vocab.cls_token,
                         sep_token=_vocab.sep_token, class_labels=None, is_test=True, vocab=_vocab)
...
def train(metric):
        ...
        if not args.only_inference:
            ...
            for batch_id, seqs in enumerate(train_data):
                ...
                if (batch_id + 1) % (args.log_interval) == 0:
                    log_train(batch_id, len(train_data), step_loss, args.log_interval, epoch_id,
    ...
    if not args.only_inference:
        ...
        with open('seed_search_' + args.task_name, 'a+') as file:
            tmp = 'Best model at epoch {}. Validation metrics:'.format(epoch_id + 1)
