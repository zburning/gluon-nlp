...
class PoolerEndLogits(HybridBlock):
    ...
    def hybrid_forward(self, F, hidden_states, start_states, start_positions, p_mask):
        ...
        if start_positions is not None:
            ...
            start_states = mx.nd.broadcast_to(start_states, shape=(bsz, slen, hsz)) # shape (bsz, slen, hsz)
...
class XLNetPoolerAnswerClass(HybridBlock):
    ...
    def __call__(self, hidden_states, start_positions=None, start_states=None, cls_index=None):
        return super(XLNetPoolerAnswerClass, self).__call__(hidden_states, start_positions, start_states, cls_index)
    ...
    def hybrid_forward(self, F, hidden_states, start_positions, start_states, cls_index):
        ...
        if start_positions is not None:
            start_states = mx.nd.gather_nd(hidden_states,
                                                mx.nd.concat(mx.nd.arange(bsz).expand_dims(1),
                                                             start_positions.reshape((bsz, 1))).T)
...
class XLNetForQA(Block):
    ...
    def forward(self, inputs, token_types, valid_length, p_mask, label, is_impossible, mems, is_evaluation):
        ...
        if not is_evaluation:
            ...
            start_top_log_probs, start_top_index = mx.ndarray.topk(start_log_probs, k=self.start_top_n,
                                                              axis=-1, ret_typ='both')  # shape (bsz, start_n_top)
            index = mx.nd.concat(*[mx.nd.arange(bsz).expand_dims(1)] * self.start_top_n).reshape(bsz * self.start_top_n, 1)
            ...
            start_states = mx.nd.gather_nd(output, gather_index).reshape((bsz, self.start_top_n, hsz)) #shape (bsz, start_n_top, hsz)
            start_states = start_states.expand_dims(1)
            start_states = mx.nd.broadcast_to(start_states, (bsz, slen, self.start_top_n, hsz)) # shape (bsz, slen, start_n_top, hsz)
            ...
            end_top_log_probs, end_top_index = mx.ndarray.topk(end_log_probs, k=self.end_top_n,
                                                          axis=1, ret_typ='both')  # shape (bsz, end_n_top, start_n_top)
